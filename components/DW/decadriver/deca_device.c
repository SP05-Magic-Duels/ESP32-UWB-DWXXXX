...
int dwt_configure(dwt_config_t *config)
{
    ...
    dwt_write16bitoffsetreg( AGC_CFG_STS_ID, 0x4, agc_config.target[prfIndex]);
		...
		// Turn off AGC
		// dwt_write16bitoffsetreg( AGC_CFG_STS_ID, AGC_CTRL1_OFFSET, 0x1);
...
uint32 _dwt_otpsetmrregs(int mode)
{
    ...
    //_dbg_printf("OTP SET MR: Setting MR,MRa,MRb for mode %2x\n",mode);
    ...
    case 0x5 :
        ...
        break;
    default :
    //  _dbg_printf("OTP SET MR: ERROR : Invalid mode selected\n",mode);
        break;
    }
...
uint32 _dwt_otpprogword32(uint32 data, uint16 address)
{
    ...
    {
        //_dbg_printf("OTP PROG 32: ERROR VPP NOT OK, programming will fail. Are MR/MRA/MRB set?\n");
        return DWT_ERROR;
    }
...
void dwt_isr(void) // assume interrupt can supply context
{
    ...
    if(status & SYS_STATUS_RXFCG) // Receiver FCS Good
	{
		...
		{
			//_dbg_printf("NO LDE done or LDE error\n");
			if(!(dw1000local.sysCFGreg & SYS_CFG_RXAUTR))
			{
				dwt_forcetrxoff();
			}
