 ...
 * Converted for ESP-IDF compatibility.
 *
 ...
 *
 * @file DW1000.cpp
 * ESP-IDF driver library (source file) for the Decawave DW1000 UWB transceiver IC.
 */
...
#include "esp_log.h"
	...
	static spi_device_handle_t dw1000_spi_device = NULL;
...
*/
// SPI settings (ESP-IDF style - frequencies in Hz)
#define DW1000_SPI_FAST_FREQ_HZ (16000000) // 16 MHz for fast operations
#define DW1000_SPI_SLOW_FREQ_HZ (2000000)  // 2 MHz for initialization
uint32_t DW1000Class::_currentSPIFreq = DW1000_SPI_FAST_FREQ_HZ;

// Placeholder SPISettings-like structure for backward compatibility
const SPISettings DW1000Class::_fastSPI = SPISettings(DW1000_SPI_FAST_FREQ_HZ, MSBFIRST, SPI_MODE0);
const SPISettings DW1000Class::_slowSPI = SPISettings(DW1000_SPI_SLOW_FREQ_HZ, MSBFIRST, SPI_MODE0);
...
{
	// Deinitialize ESP-IDF SPI device
	if (dw1000_spi_device != NULL)
	{
		spi_bus_remove_device(dw1000_spi_device);
		dw1000_spi_device = NULL;
	}
}
...
{
	_ss = ss;
	// Configure CS pin as output using ESP-IDF
	gpio_set_direction((gpio_num_t)_ss, GPIO_MODE_OUTPUT);
	gpio_set_level((gpio_num_t)_ss, 1); // Set CS high (inactive state)
}
...
void DW1000Class::begin(uint8_t irq, uint8_t rst)
{
	...
	vTaskDelay(pdMS_TO_TICKS(5));
	// Configure the IRQ pin as INPUT using ESP-IDF
	gpio_set_direction((gpio_num_t)irq, GPIO_MODE_INPUT);
	// SPI initialization is handled externally in ESP-IDF
	// #ifndef ESP8266
	...
	// attach interrupt using ESP-IDF GPIO interrupt handler
	// TODO: Implement GPIO interrupt handler for ESP-IDF if needed
	// gpio_install_isr_service(0);
	// gpio_set_intr_type((gpio_num_t)_irq, GPIO_INTR_RISING);
	// gpio_isr_handler_add((gpio_num_t)_irq, handleInterrupt, NULL);
}
...
void DW1000Class::convertToByte(char string[], uint8_t *uint8_ts)
{
	uint8_t eui_uint8_t[LEN_EUI];
	// we fill it with the char array under the form of "AA:FF:1C:...."
	for (uint16_t i = 0; i < LEN_EUI; i++)
	{
		eui_uint8_t[i] = (nibbleFromChar(string[i * 3]) << 4) + nibbleFromChar(string[i * 3 + 1]);
	}
	memcpy(uint8_ts, eui_uint8_t, LEN_EUI);
}
...
void DW1000Class::commitConfiguration()
{
	...
	tune();
	// TODO check not larger two uint8_ts integer
...
{
	uint16_t n = data.size() + 1; // same logic as Arduino: include null terminator
	uint8_t *dataBytes = (uint8_t *)malloc(n);

	// Equivalent of Arduino String.getBytes()
	memcpy(dataBytes, data.data(), data.size());
	dataBytes[data.size()] = '\0'; // ensure null termination
...
void DW1000Class::getData(const std::string &data)
{
	...
	// clear the string (replacement for Arduino String.remove and = "")
	data.clear();

	// append bytes just like original code
	for (uint16_t i = 0; i < n; i++)
	{
		data += static_cast<char>(dataBytes[i]);
	}
...
/*
 * Set the value of a bit in an array of uint8_ts that are considered
 * consecutive and stored from MSB to LSB.
 * @param data
 * 		The number as uint8_t array.
 * @param n
 * 		The number of uint8_ts in the array.
 * @param bit
...
/*
 * Check the value of a bit in an array of uint8_ts that are considered
 * consecutive and stored from MSB to LSB.
 * @param data
 * 		The number as uint8_t array.
 * @param n
...
/*
 * Read uint8_ts from the DW1000. Number of uint8_ts depend on register length.
 * @param cmd
 * 		The register address (see Chapter 7 in the DW1000 user manual).
 * @param data
 *		The data array to be read into.
 * @param n
 *		The number of uint8_ts expected to be received.
 */
// TODO incomplete doc
void DW1000Class::readBytes(uint8_t cmd, uint16_t offset, uint8_t data[], uint16_t n)
{
	...
	// ESP-IDF SPI transaction
	gpio_set_level((gpio_num_t)_ss, 0); // CS low
	spi_transaction_t t;
	memset(&t, 0, sizeof(t));

	// Send header
	t.length = headerLen * 8; // length in bits
	t.tx_buffer = header;
	t.rx_buffer = NULL;
	spi_device_transmit(dw1000_spi_device, &t);

	// Read data
	memset(&t, 0, sizeof(t));
	t.length = n * 8; // length in bits
	t.tx_buffer = NULL;
	t.rx_buffer = data;
	spi_device_transmit(dw1000_spi_device, &t);

	esp_rom_delay_us(5);
	gpio_set_level((gpio_num_t)_ss, 1); // CS high
}

// always 4 uint8_ts
// TODO why always 4 uint8_ts? can be different, see p. 58 table 10 otp memory map
...
/*
 * Write uint8_ts to the DW1000. Single uint8_ts can be written to registers via sub-addressing.
 * @param cmd
 * 		The register address (see Chapter 7 in the DW1000 user manual).
 * @param offset
 ...
 * @param data_size
 *		The number of uint8_ts to be written (take care not to go out of bounds of
 * 		the register).
 */
...
void DW1000Class::writeBytes(uint8_t cmd, uint16_t offset, uint8_t data[], uint16_t data_size)
{
	...
	// Write data
	memset(&t, 0, sizeof(t));
	t.length = data_size * 8; // length in bits
	t.tx_buffer = data;
	t.rx_buffer = NULL;
	spi_device_transmit(dw1000_spi_device, &t);
...
{
	uint16_t i, j, b;
	b = sprintf(msgBuffer, "Data, uint8_ts: %d\nB: 7 6 5 4 3 2 1 0\n", n); // TODO - type
	for (i = 0; i < n; i++)
	{
...
void DW1000Class::getPrettyBytes(uint8_t cmd, uint16_t offset, char msgBuffer[], uint16_t n)
{
	...
	b = sprintf(msgBuffer, "Reg: 0x%02x, uint8_ts: %d\nB: 7 6 5 4 3 2 1 0\n", cmd, n); // TODO - tpye
	for (i = 0; i < n; i++)
	{
