 ...
 * ESP-IDF global library (source file) working with the DW1000 library
...
#include "esp_log.h"
	...
	static const char *DEBUG_HEADER = "DW1000Ranging";
	...
	void DW1000RangingClass::configureNetwork(uint16_t deviceAddress, uint16_t networkId, const uint8_t mode[])
	{
	...
	void DW1000RangingClass::generalStart()
	{
		...
		{
			// DEBUG monitoring
			ESP_LOGI(DEBUG_HEADER, "DW1000-arduino\n");
			// initialize the driver

			ESP_LOGI(DEBUG_HEADER, "configuration..\n");
			// DEBUG chip info and registers pretty printed
			char msg[90];
			DW1000.getPrintableDeviceIdentifier(msg);
			ESP_LOGI(DEBUG_HEADER, "Device ID: ");
			ESP_LOGI(DEBUG_HEADER, "%s\n", msg);
			DW1000.getPrintableExtendedUniqueIdentifier(msg);
			ESP_LOGI(DEBUG_HEADER, "Unique ID: ");
			ESP_LOGI(DEBUG_HEADER, "%s\n", msg);
			char string[6];
			sprintf(string, "%02X:%02X", _currentShortAddress[0], _currentShortAddress[1]);
			ESP_LOGI(DEBUG_HEADER, " short: ");
			ESP_LOGI(DEBUG_HEADER, "%s\n", string);

			DW1000.getPrintableNetworkIdAndShortAddress(msg);
			ESP_LOGI(DEBUG_HEADER, "Network ID & Device Address: ");
			ESP_LOGI(DEBUG_HEADER, "%s\n", msg);
			DW1000.getPrintableDeviceMode(msg);
			ESP_LOGI(DEBUG_HEADER, "Device mode: ");
			ESP_LOGI(DEBUG_HEADER, "%s\n", msg);
		}
		...
		_rangingCountPeriod = (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS);
	}

	void DW1000RangingClass::startAsAnchor(char address[], const uint8_t mode[], const bool randomShortAddress)
	{
		// save the address
		DW1000.convertToByte(address, _currentAddress);
		// write the address on the DW1000 chip
		DW1000.setEUI(address);
		ESP_LOGI(DEBUG_HEADER, "device address: %s", address);
		if (randomShortAddress)
		{
			// we need to define a random short address using ESP-IDF:
			_currentShortAddress[0] = esp_random() & 0xFF;
			_currentShortAddress[1] = esp_random() & 0xFF;
		}
		else
		{
			// we use first two uint8_ts in addess for short address
	...
	void DW1000RangingClass::startAsTag(char address[], const uint8_t mode[], const bool randomShortAddress)
	{
		...
		ESP_LOGI(DEBUG_HEADER, "### DW_TAG_R ###");
	}
	...
	DW1000Device *DW1000RangingClass::searchDistantDevice(uint8_t shortAddress[])
	{
		// we compare the 2 uint8_ts address with the others
		for (uint16_t i = 0; i < _networkDevicesNumber; i++)
		{ // TODO 8bit?
			if (memcmp(shortAddress, _networkDevices[i].getuint8_tShortAddress(), 2) == 0)
			{
	...
	{
		uint32_t curMillis = (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS);
	...
	int16_t DW1000RangingClass::detectMessageType(uint8_t datas[])
	{
	...
	void DW1000RangingClass::copyShortAddress(uint8_t address1[], uint8_t address2[])
	{
		*address1 = *address2;
	...
	void DW1000RangingClass::transmitRangingInit(DW1000Device *myDistantDevice)
	{
		...
		_globalMac.generateLongMACFrame(data, _currentShortAddress, myDistantDevice->getuint8_tAddress());
		// we define the function code
		data[LONG_MAC_LEN] = RANGING_INIT;

		copyShortAddress(_lastSentToShortAddress, myDistantDevice->getuint8_tShortAddress());
	...
	void DW1000RangingClass::transmitPoll(DW1000Device *myDistantDevice)
	{
		...
		if (myDistantDevice == nullptr)
		{
			...
			for (uint8_t i = 0; i < _networkDevicesNumber; i++)
			{
				...
				memcpy(data + SHORT_MAC_LEN + 2 + 4 * i, _networkDevices[i].getuint8_tShortAddress(), 2);
		...
		else
		{
			...
			_globalMac.generateShortMACFrame(data, _currentShortAddress, myDistantDevice->getuint8_tShortAddress());
	...
	void DW1000RangingClass::transmitRange(DW1000Device *myDistantDevice)
	{
		...
		if (myDistantDevice == nullptr)
		{
			...
			{
				// we write the short address of our device:
				memcpy(data + SHORT_MAC_LEN + 2 + 17 * i, _networkDevices[i].getuint8_tShortAddress(), 2);
	...
	/* FOR DEBUGGING*/
	void DW1000RangingClass::visualizeDatas(uint8_t datas[])
	{
		char string[60];
